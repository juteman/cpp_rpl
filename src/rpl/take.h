...
public:
    ...
    auto operator()(producer<Value, Error, Generator>&& initial)
    {
        return make_producer<Value, Error>(
            [initial = std::move(initial), limit = _count](const auto& consumer) mutable {
...
inline auto take(int count) -> details::take_helper
{
    Expects(count >= 0);
...
template <typename Predicate> class take_while_helper
{
public:
    template <typename OtherPredicate> take_while_helper(OtherPredicate&& predicate) :
...
template <typename Predicate> inline auto take_while(Predicate&& predicate)
    -> details::take_while_helper<std::decay_t<Predicate>>
{
    return details::take_while_helper<std::decay_t<Predicate>>(std::forward<Predicate>(predicate));
}
