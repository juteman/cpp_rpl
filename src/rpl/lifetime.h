...
public:
    ...
    template <typename Destroy, typename = decltype(std::declval<Destroy>()())> lifetime(Destroy&& destroy);

    explicit operator bool() const { return !_callbacks.empty(); }

    template <typename Destroy, typename = decltype(std::declval<Destroy>()())> void add(Destroy&& destroy);
...
inline lifetime::lifetime(lifetime&& other) : _callbacks(details::take(other._callbacks)) {}
...
template <typename Destroy, typename> inline lifetime::lifetime(Destroy&& destroy)
{
    _callbacks.emplace_back(std::forward<Destroy>(destroy));
}

template <typename Destroy, typename> inline void lifetime::add(Destroy&& destroy)
{
    _callbacks.emplace_back(std::forward<Destroy>(destroy));
}
...
inline void lifetime::add(lifetime&& other)
{
    ...
    _callbacks.insert(_callbacks.end(), std::make_move_iterator(callbacks.begin()),
...
inline void lifetime::destroy()
{
    ...
    for (auto i = callbacks.rbegin(), e = callbacks.rend(); i != e; ++i) { (*i)(); }
}

inline void lifetime::release() { _callbacks.clear(); }
