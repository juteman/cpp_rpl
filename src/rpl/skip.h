...
public:
    ...
    auto operator()(producer<Value, Error, Generator>&& initial)
    {
        return make_producer<Value, Error>(
            [initial = std::move(initial), skipping = _count](const auto& consumer) mutable {
                ...
                auto initial_consumer = make_consumer<Value, Error>(
                    [consumer, count](auto&& value) {
                        if (*count) { --*count; }
                        else { consumer.put_next_forward(std::forward<decltype(value)>(value)); }
                    },
...
inline auto skip(int count) -> details::skip_helper
{
    Expects(count >= 0);
