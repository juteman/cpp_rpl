...
#define AssertValidationCondition(condition, message, file, line) \
    ((AssertUnlikelyHelper(!(condition))) ? ::base::assertion::fail(message, file, line) : ::base::assertion::noop())

#define SOURCE_FILE_BASENAME (::base::assertion::extract_basename(__FILE__, sizeof(__FILE__)))

#define AssertCustom(condition, message) (AssertValidationCondition(condition, message, SOURCE_FILE_BASENAME, __LINE__))
...
#    undef Expects
#endif // Expects
#define Expects(condition) (AssertValidationCondition(condition, "\"" #condition "\"", SOURCE_FILE_BASENAME, __LINE__))

#ifdef Ensures
#    undef Ensures
#endif // Ensures
#define Ensures(condition) (AssertValidationCondition(condition, "\"" #condition "\"", SOURCE_FILE_BASENAME, __LINE__))

#ifdef Unexpected
#    undef Unexpected
#endif // Unexpected
#define Unexpected(message) (::base::assertion::fail("Unexpected: " message, SOURCE_FILE_BASENAME, __LINE__))
